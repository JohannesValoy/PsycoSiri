...
class AgentIndicator extends PanelMenu.Button {
    ...
    _buildAgentCard(agent) {
        ...
        if (toolCalls.length > 0 || errors.length > 0) {
            ...
            // Tool body (hidden by default — collapsed)
    ...
    _onAgentStateChange(_runId, run) {
        const runningCount = this._agentManager.getRunningCount();

        if (runningCount > 0) {
            this._badge.set_text(`${runningCount}`);
            this._badge.show();
            this._startSpinner();
        } else {
            this._badge.hide();
        ...
        // Rebuild on terminal states immediately; debounce during running to
        // keep the todo checklist up-to-date without closing the popup.
        if (this.menu.isOpen) {
            if (run.state === 'completed' || run.state === 'error') {
                ...
                this._rebuildDebounceId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 2000, () => {
                    this._rebuildDebounceId = null;
                    if (this.menu.isOpen)
                        this._rebuildAgentList();
                    return GLib.SOURCE_REMOVE;
