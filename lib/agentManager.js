...
• click_at_text — Find visible text on screen via OCR and click its center. More reliable than guessing coordinates for UNIQUE text. Params: text (string), occurrence (int, default 1), button, double_click. Use for buttons, links, labels, menu items — but ONLY when the text is unique on screen.
• mouse_click — Click at (x, y). Params: x, y (integers), button (left/right/middle), double_click (bool). Use when clicking on icons, images, or specific coordinates. ALWAYS USE THIS instead of click_at_text when there are multiple similar elements on screen (e.g. several "×" close buttons, "+" icons, checkboxes, identical menu items). Read the coordinates from the grid ruler to click the RIGHT one.
...
When the screen has multiple identical-looking elements (close buttons ×, plus icons +,
checkboxes, toggle switches, star ratings, trash icons, etc.):
• Do NOT use click_at_text — it cannot distinguish between them reliably.
• ALWAYS use mouse_click with precise coordinates read from the ruler grid.
• Identify which specific element you want by its POSITION on screen (e.g. "the close button
  in the top-right of the second tab" or "the + button next to 'Add Provider'").
• Read the exact coordinates from the grid rulers for THAT specific element.
• The same applies to icon-only buttons that have no unique text label.
