...
export class AgentManager {
    ...
    async spawnAgent(agentConfigId, task, sessionId = null) {
        ...
        if (agentCfg.backupProviderId && agentCfg.backupModelId) {
            const backupProvCfg = providerConfigs[agentCfg.backupProviderId];
            if (backupProvCfg) {
                backupProvider = new AIProvider(
                    agentCfg.backupProviderId,
                    backupProvCfg.name,
                    backupProvCfg.apiKey,
                    backupProvCfg.baseUrl
                );
                backupProvider.setModel(agentCfg.backupModelId);
            }
        }
    ...
    async _executeAgentLoop(run) {
        ...
        try {
            ...
            while (iterations < MAX_AGENT_TOOL_ITERATIONS) {
                ...
                // Prune context if it's getting too large to avoid API timeouts.
                // Estimate total chars; if over threshold, truncate old tool results.
                const MAX_CONTEXT_CHARS = 120000;
                let totalChars = run.systemPrompt.length;
                for (const m of run.messages) {
                    const c = typeof m.content === 'string' ? m.content : JSON.stringify(m.content);
                    totalChars += c.length;
                }

                let prunedMessages = run.messages;
                if (totalChars > MAX_CONTEXT_CHARS) {
                    prunedMessages = [];
                    // Always keep first 4 messages (user task + plan_tasks + result + first assistant)
                    const keepStart = Math.min(4, run.messages.length);
                    // Always keep last 20 messages (recent context)
                    const keepEnd = Math.min(20, run.messages.length);
                    const cutStart = keepStart;
                    const cutEnd = run.messages.length - keepEnd;
                    ...
                    for (let i = 0; i < run.messages.length; i++) {
                        ...
                        if (i < cutStart || i >= cutEnd) {
                            ...
                            if (content.length > 200) {
                                prunedMessages.push({
                                    ...msg,
                                    content: content.slice(0, 150) + `\n... [truncated ${content.length} chars to save context]`,
                                });
                            } else {
                                prunedMessages.push(msg);
                            }
