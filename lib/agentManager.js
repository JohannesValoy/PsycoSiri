...
- If a tool returns an error, try an alternative approach, then move on or finish with what you have.`;
        ...
        let basePrompt;
        if (agentCfg.systemPrompt && !isRepairTask) {
            basePrompt = agentCfg.systemPrompt;
        } else if (isComputerUseAgent) {
...
• Screenshots may have a coordinate grid overlay with labeled intersections (e.g. "A1", "B3")
  Grid labels help you estimate positions. Column letters go left-to-right (A=100, B=200, ...),
  row numbers go top-to-bottom (1=100, 2=200, ...). So "C5" ≈ (300, 500).
...
for c in range(1, min(cols + 1, 27)):
    for r in range(1, rows + 1):
        label = chr(64 + c) + str(r)
...
for c in range(1, min(cols + 1, 27)):
    ...
    cr.set_source_rgba(0.15, 0.3, 0.5, 0.7)
                    ...
                    try {
                        ...
                        if (initImage) {
                            ...
                            const gridNote = gridEnabled
                                ? `\n\nA coordinate grid is overlaid on the screenshot. Grid lines are ${gridSpacing}px apart. Labels like "A1" mark intersection at (${gridSpacing}, ${gridSpacing}), "B3" at (${gridSpacing * 2}, ${gridSpacing * 3}), etc. Column letters run left-to-right (A, B, C...), row numbers run top-to-bottom (1, 2, 3...). Use these as a guide to estimate precise pixel coordinates for clicks.`
                                : '';
                            run.messages.push({
                                role: 'user',
                                content: [
