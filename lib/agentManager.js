...
Reply: VERDICT:CONTINUE or VERDICT:CANCEL plus a one-line reason.`;
    ...
    _attemptAutoRepair(run) {
        ...
        const repairTask = `AUTO-REPAIR: Diagnose why a previous agent failed. You have 5 MINUTES MAX.

CRITICAL RULES:
- You are a DIAGNOSTIC agent. Your ONLY job is to figure out WHY it failed and report.
- Do NOT redo or continue the original task. Do NOT read files unrelated to the error.
- Be fast: read the log, check 1-2 relevant source files if needed, then call finish.
- You MUST call finish within a few tool calls. If unsure, finish with not_relevant.
...
DIAGNOSE, then call finish with EXACTLY one of:
- "OUTCOME:fixed REASON:<what you fixed>" — only if you found AND fixed a code bug
- "OUTCOME:not_relevant REASON:<explanation>" — if it's transient (API timeout, model stall, etc.)
- "OUTCOME:failed REASON:<what went wrong>" — if you found a bug but couldn't fix it`;
