...
KEEP MOVING — every response MUST include at least one tool call.`;
    ...
    _drawGridOverlay(base64Png) {
        try {
            ...
            // strips for axis labels. No intersection labels — much cleaner.
            const script = `
import cairo, sys, gi
...
# Ruler dimensions
TOP_H = 24   # top ruler height
LEFT_W = 44  # left ruler width
...
for c in range(1, cols + 1):
    x = c * spacing
    cr.move_to(x, TOP_H)
    cr.line_to(x, TOP_H + TICK_LEN)
    cr.stroke()
for r in range(1, rows + 1):
    y = r * spacing
    cr.move_to(LEFT_W, y)
    cr.line_to(LEFT_W + TICK_LEN, y)
    cr.stroke()

# ── 4. Draw X-axis labels in top ruler ──
...
for c in range(1, cols + 1):
    ...
    cr.move_to(cx - log.width / 2, (TOP_H - log.height) / 2)
    PangoCairo.show_layout(cr, layout)

# ── 5. Draw Y-axis labels in left ruler ──
for r in range(1, rows + 1):
    label = str(r * spacing)
    cy = r * spacing
    layout.set_text(label, -1)
    ink, log = layout.get_pixel_extents()
    cr.set_source_rgba(1, 1, 1, 0.95)
    cr.move_to((LEFT_W - log.width) / 2, cy - log.height / 2)
    PangoCairo.show_layout(cr, layout)
