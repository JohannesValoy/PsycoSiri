...
export class MemoryManager {
    ...
    async _migrateIfNeeded() {
        ...
        // Add tool_call_id column to conversations table for storing tool result messages
        try {
            const cols = await this._exec(
                "PRAGMA table_info(conversations);"
            );
            if (!cols.includes('tool_call_id')) {
                await this._exec(
                    "ALTER TABLE conversations ADD COLUMN tool_call_id TEXT;"
                );
            }
        } catch (e) {
            console.error(`[Aether] conversations tool_call_id migration error: ${e.message}`);
        }
        ...
        // Ensure agent_runs table exists (for persistent agent history)
        try {
            await this._exec(`
            ...
            console.error(`[Aether] agent_runs migration error: ${e.message}`);
        }
    }

    /**
     * Execute raw SQL against the database (with retry for DB locked).
     */
    async _exec(sql) {
        for (let attempt = 0; attempt <= DB_RETRY_COUNT; attempt++) {
            ...
            if (exitStatus === 0)
                return stdout;
            if (attempt < DB_RETRY_COUNT && /locked|busy/i.test(stderr)) {
                await new Promise(resolve => {
                    GLib.timeout_add(GLib.PRIORITY_DEFAULT, DB_RETRY_DELAY_MS * (attempt + 1), () => {
                        resolve();
    ...
    /**
     * Execute SQL and return results as JSON array (with retry for DB locked).
     */
    async _query(sql) {
        for (let attempt = 0; attempt <= DB_RETRY_COUNT; attempt++) {
            const {stdout, stderr, exitStatus} = await runSubprocess(
    ...
    async storeMessage(sessionId, role, content, toolCalls = null, tokenEstimate = 0, toolCallId = null) {
        const tcJson = toolCalls ? sqlEscape(JSON.stringify(toolCalls)) : 'NULL';
        const tcIdSql = toolCallId ? sqlEscape(toolCallId) : 'NULL';
        const sql = `INSERT INTO conversations (session_id, role, content, tool_calls, tool_call_id, token_estimate)
            VALUES (${sqlEscape(sessionId)}, ${sqlEscape(role)}, ${sqlEscape(content)}, ${tcJson}, ${tcIdSql}, ${tokenEstimate});`;
        await this._exec(sql);
    }
    ...
    // --- Agent Runs (persistent history) ---

    async storeAgentRun(id, agentConfigId, task, sessionId = null) {
        const sql = `INSERT OR REPLACE INTO agent_runs (id, agent_config_id, task, session_id, started_at)
            VALUES (${sqlEscape(id)}, ${sqlEscape(agentConfigId)}, ${sqlEscape(task)},
            ${sessionId ? sqlEscape(sessionId) : 'NULL'}, datetime('now'));`;
        await this._exec(sql);
    }

    async updateAgentRun(id, state, result = null, error = null) {
        const resultSql = result !== null ? `, result = ${sqlEscape(result)}` : '';
        const errorSql = error !== null ? `, error = ${sqlEscape(error)}` : '';
        const completedSql = (state === 'completed' || state === 'error' || state === 'cancelled')
            ? `, completed_at = datetime('now')` : '';
        const sql = `UPDATE agent_runs SET state = ${sqlEscape(state)}${resultSql}${errorSql}${completedSql} WHERE id = ${sqlEscape(id)};`;
        await this._exec(sql);
    }

    async getAgentRuns(sessionId) {
        return this._query(
            `SELECT * FROM agent_runs WHERE session_id = ${sqlEscape(sessionId)} ORDER BY started_at DESC;`
        );
    }

    async getRecentAgentRuns(limit = 20) {
        return this._query(
    ...
    async getAgentRun(id) {
        const rows = await this._query(
            `SELECT * FROM agent_runs WHERE id = ${sqlEscape(id)} LIMIT 1;`
        );
        return rows.length > 0 ? rows[0] : null;
    }
