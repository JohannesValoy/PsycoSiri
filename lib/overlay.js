...
const MESSAGE_PANEL_MAX_HEIGHT_FRACTION = 0.52;
...
class AetherOverlay extends St.Widget {
    ...
    _buildUI() {
        ...
        this._outputBox.get_layout_manager().spacing = 6;
        ...
        // ── Glow animations ──
        // Aurora on message panel
        this._msgAuroraAnim = new GlowAnimation(this._messagePanel, 'aether-aurora', false);
        // Aurora on input bar
    ...
    _positionPanels() {
        ...
        const maxH = Math.round(monitor.height * MESSAGE_PANEL_MAX_HEIGHT_FRACTION);
    ...
    open() {
        ...
        // Start aurora glow on both panels
        GLib.timeout_add(GLib.PRIORITY_DEFAULT, 250, () => {
            if (this._isOpen) {
                this._msgAuroraAnim.setState(AnimationState.IDLE);
    ...
    close() {
        ...
        this._holdToTalk = false;
        this._discardSttCallback = true;
        this._msgAuroraAnim.setState(AnimationState.OFF);
    ...
    _setGlowState(state) {
        this._msgAuroraAnim.setState(state);
    ...
    destroy() {
        ...
        if (this._msgAuroraAnim)
            this._msgAuroraAnim.destroy();
