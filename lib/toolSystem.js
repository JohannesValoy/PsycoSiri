...
import {ShellCommandTool} from '../tools/shellCommand.js';
import {ReadFileTool, WriteFileTool, ListDirectoryTool} from '../tools/fileOps.js';
...
export class ToolRegistry {
    ...
    registerBuiltins(memory, todoManager, settings, agentManager = null) {
        ...
        this.register({
            name: 'memory_store',
            description: 'Store a memory/fact for long-term recall. Types: fact, preference, procedure, note.',
            parameters: {
                type: 'object',
                properties: {
                    type: {type: 'string', enum: ['fact', 'preference', 'procedure', 'note']},
        ...
        if (agentManager) {
            this.register({
                ...
                execute: async (args) => {
                    ...
                    try {
                        const runId = agentManager.spawnAgent(args.agent_id, args.task);
