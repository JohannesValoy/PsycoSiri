...
import {getConfigDir, getMemoryDir, readFileAsync} from './utils.js';
...
export class ToolRegistry {
     ...
     */
    registerBuiltins(memory, todoManager, settings, agentManager = null, getSessionId = null) {
        ...
        this.register({
            name: 'save_to_memory',
            description: 'Append content to a memory file. Use "MEMORY.md" for the main agent memory, or "projects/<name>.md" for per-project memory files. Creates the file if it does not exist. Content is appended with a newline separator.',
            parameters: {
                type: 'object',
                properties: {
                    file: {type: 'string', description: 'Memory file path relative to memory directory (e.g., "MEMORY.md", "projects/my-app.md")'},
                    content: {type: 'string', description: 'Content to append (markdown format recommended)'},
                },
            ...
            execute: async (args) => {
                ...
                const sanitized = args.file.replace(/\.\./g, '').replace(/^\//, '');
                const filePath = GLib.build_filenamev([memDir, sanitized]);

                // Ensure parent directory exists
                const parentDir = GLib.path_get_dirname(filePath);
                const parentFile = Gio.File.new_for_path(parentDir);
                if (!parentFile.query_exists(null))
                    parentFile.make_directory_with_parents(null);

                const file = Gio.File.new_for_path(filePath);
                let existing = '';
                if (file.query_exists(null)) {
                    try {
                        existing = await readFileAsync(filePath);
                    } catch {
                        existing = '';
                    }
                }
                ...
                const newContent = existing + separator + args.content + '\n';
                file.replace_contents(
                    new TextEncoder().encode(newContent),
                    null, false, Gio.FileCreateFlags.REPLACE_DESTINATION, null
                );
                return `Saved to memory file: ${sanitized} (${newContent.length} bytes total)`;
            },
        });

        this.register({
            name: 'read_memory_file',
            description: 'Read the contents of a memory file. Use "MEMORY.md" for the main memory, or "projects/<name>.md" for project-specific memory.',
            parameters: {
                type: 'object',
                properties: {
                    file: {type: 'string', description: 'Memory file path relative to memory directory (e.g., "MEMORY.md", "projects/my-app.md")'},
            ...
            execute: async (args) => {
                ...
                const file = Gio.File.new_for_path(filePath);
                if (!file.query_exists(null))
                    return `Memory file not found: ${sanitized}`;
                try {
                    return await readFileAsync(filePath);
                } catch (e) {
                    return `Error reading memory file: ${e.message}`;
                }
            },
        });
        ...
        this.register({
            ...
            description: 'List all available memory files in the agent memory directory.',
            parameters: {
                type: 'object',
                properties: {},
            },
            execute: async () => {
                ...
                const dir = Gio.File.new_for_path(memDir);
                ...
                while ((info = enumerator.next_file(null)) !== null) {
                    ...
                    if (type === Gio.FileType.REGULAR)
                        results.push(name);
                    else if (type === Gio.FileType.DIRECTORY && name === 'projects') {
                        // List project files
                        const projDir = Gio.File.new_for_path(GLib.build_filenamev([memDir, 'projects']));
                        const projEnum = projDir.enumerate_children('standard::name', Gio.FileQueryInfoFlags.NONE, null);
                        let pInfo;
                        while ((pInfo = projEnum.next_file(null)) !== null)
                            results.push(`projects/${pInfo.get_name()}`);
                    }
                }

                if (results.length === 0)
                    return 'No memory files found. Use save_to_memory to create one.';
            ...
            this.register({
                ...
                execute: async (args) => {
                    ...
                    try {
                        ...
                        const runId = await agentManager.spawnAgent(args.agent_id, args.task, sessionId);
